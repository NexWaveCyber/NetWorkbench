...
public enum SNMPv3Crypto {
    ...
    /// RFC 3414 Password-to-Key Localization Algorithm (A.2)
    ...
    public static func passwordToKey(
        ...
        // 1. Expand password to 1,048,576 octets (1MB) and hash iteratively
        ...
        case .md5:
            ...
            // Localize key with engineID: Hash(digest + engineID + digest)
    ...
    public static func computeAuthHMAC(
        ...
        case .sha256:
            ...
            // Truncate to 16 bytes for SHA-256
            return Data(mac.prefix(16))
        }
    }

    /// Encrypt ScopedPDU payload using AES-128 CFB mode (RFC 3826)
    public static func encryptAES128(
        ...
        let key = [UInt8](privKey.prefix(16))
        guard key.count == 16 else {
            throw NSError(domain: "SNMPv3Crypto", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid AES-128 key length (expected 16 bytes)"])
        }
        ...
        // privParams on the wire is the 8-byte salt
        let privParams = Data(bytes: &saltBE, count: 8)

        let ciphertext = try cryptAES128CFB(data: payload, key: Data(key), iv: iv, operation: CCOperation(kCCEncrypt))
        return (ciphertext, privParams)
    }

    /// Decrypt ScopedPDU payload using AES-128 CFB mode
    public static func decryptAES128(
        ...
        let key = [UInt8](privKey.prefix(16))
        guard key.count == 16 else {
            throw NSError(domain: "SNMPv3Crypto", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid AES-128 key length"])
        }
        ...
        return try cryptAES128CFB(data: ciphertext, key: Data(key), iv: iv, operation: CCOperation(kCCDecrypt))
    }

    private static func cryptAES128CFB(
        data: Data,
        key: Data,
        iv: Data,
        ...
        guard status == kCCSuccess, let cryptorRef = cryptor else {
            throw NSError(domain: "SNMPv3Crypto", code: Int(status), userInfo: [NSLocalizedDescriptionKey: "Failed to initialize AES-128-CFB cryptor (status: \(status))"])
        }
        ...
        guard updateStatus == kCCSuccess else {
            throw NSError(domain: "SNMPv3Crypto", code: Int(updateStatus), userInfo: [NSLocalizedDescriptionKey: "AES-128-CFB update failed (status: \(updateStatus))"])
        }
        ...
        guard finalStatus == kCCSuccess else {
            throw NSError(domain: "SNMPv3Crypto", code: Int(finalStatus), userInfo: [NSLocalizedDescriptionKey: "AES-128-CFB final failed (status: \(finalStatus))"])
        }
